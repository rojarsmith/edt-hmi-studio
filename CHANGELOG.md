...
### Added
- **A logic graph can name the screens it is active on** — a Properties panel joins the right side above Variables, the same collapsible shape as the graph list, showing the open graph's settings. Active on Screens defaults to all; leaving "all" starts from an explicit list with every screen still checked, so excluding one screen is one click rather than a rebuild, and an empty selection says plainly that the graph runs on no screen. The choice is stored on the graph and survives save and export. Generated code does not gate on it yet — the setting is the contract, the gating is the follow-up
...
### Fixed
- **An Event Trigger graph never ran on hardware — the dialog never asked which widget fires it** — the whole chain existed and was tested (the data model carries a target component, codegen emits `lv_obj_add_event_cb`, every firmware template calls `ui_logic_init()` at the right moment), but the Edit Node dialog offered only Event Type, so the registration line was never generated and every event graph was dead code. The dialog now has the same Target Component selector its sibling nodes always had, and warns plainly while none is chosen. The node face says who too: the chosen component's name sits above the event line, and a target whose component was later deleted reads `Target: (missing)` instead of letting codegen fall back silently. The **Event Object** output steps behind Factory Dev Mode in the same change: generated code still discards the event, so in normal mode the port only promised what the device cannot deliver — one already wired stays visible in both modes so a connection is never stranded. The investigation and both decisions are recorded in [docs/logic-event-trigger.md](docs/logic-event-trigger.md)
