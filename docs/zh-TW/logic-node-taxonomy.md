...
狀態：**四步已於 2026-08-16 全部落地 —— 本文自此成為歷史記錄，保留的是
推理過程。**
...
2. **已完成（2026-08-16）：** 依上表重新分組調色盤。每個定義帶一個純顯示的
   `paletteGroup`；儲存的節點 type、由它決定的顏色、以及每一張已存的圖都
   原封不動，並有測試同時釘住架子內容與儲存 type 的數量，兩層不可能無聲
   漂移
3. **已完成（2026-08-16）：** Custom 架子只在原廠人員研發模式存在 ——
   調色盤與搜尋一體適用；已放進圖裡的節點在任何模式都照常渲染與產生
4. **已完成（2026-08-16）：** 儲存的分類正名為六個架子，顯示分組與資料
   從此一致，`paletteGroup` 功成身退。節點顏色跟著架子走 —— Compare 轉紫、
   Delay 轉琥珀、tag 節點轉青、Call Function 轉灰。遷移方案：
   `normalizeLogicGraphs` 在 store 的每個入口（開專案、專案清單、匯入圖）
   以 subType（權威來源）重新推導每個節點的分類，帶著 `condition`、
   `action` 的舊檔永遠讀得進來；未知的 subType 保留仍然有效的儲存值，
   否則退到 `custom`。伺服器端程式碼產生不需要遷移：它全面以 subType
   為準，僅有的一處分類判斷（無 trigger 的線性後備）改以定義表的執行
   輸入埠判定 —— 順帶修好它從前看不見 `var_write` 與 `tag_write` 的盲點
