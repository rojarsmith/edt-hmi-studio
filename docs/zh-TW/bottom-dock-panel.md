...
**其實是三個狀態，不是兩個，而且只有一個被儲存。** `dockStore.expanded` 是作者的選擇
——展開，還是收到只剩分頁列。第三個是 *inert*：離開 Deploy 分頁之後，那條帶子**留在
原地**、把頁簽清空，右側的箭頭變暗且按不下去。這個狀態由 `App` 從「哪個分頁」與「有沒有
在跑」推導出來，從不儲存，所以離開分頁不會花掉作者的展開狀態——回來就還原。

**留著那條帶子而不是把它拿掉**，正是為了讓工作區不會每次換分頁就跳動一個帶子的高度；
而那個變暗的箭頭說的是「面板還在，只是現在不能用」，不是「面板不見了」。帶子的高度是在
CSS 裡釘死的，不是由內容推導出來的——因為清空頁簽本來會讓它縮水：實測收合 29 px、inert
29 px、展開 220 px，`.app-body` 把每一次的差額都吸收掉，狀態列從頭到尾沒有移動過。
