...
**它目前在兩種模式下都保留完整內容。** 這個 pane 按定義就是工程視圖——是「Work 那一行階段
不夠用時」該去看的地方——所以它沒有像 Emulator 自己的介面那樣為原廠研發模式改寫用語
（[factory-dev-mode.md](./factory-dev-mode.md)）。預期的終局是整個頁簽移到那個旗標後面，
那才是真正回答了這個問題，而不是硬把一份編譯器逐字稿翻成它沒有的字。

**仍然沒做。** 頁簽列的方向鍵導航，以及拖曳把手的鍵盤替代方案。四個 pane 只會讓這件事
比三個更急，不會更不急。

## 12. 第五個 pane：Information

第一個不是 log 的 pane。**Information** 說的是被選取的元件需要面板提供什麼——設計者必須
知道、卻從畫布上讀不出來的規則——以及眼前這一個違反了哪些。

**它從哪裡來。** Video 元件。影片在面板上會遮住所有跟它重疊的東西、以錄製時的尺寸而不是
元件的尺寸顯示、播的是一個編輯器從來沒看過的卡片上的檔案。這些在畫布上都看不見，畫布上
的影片就是一塊寫著檔名的黑框。第一個放了影片的專案把一顆按鈕疊在上面，面板顯示了影片卻
沒有按鈕，而設計者沒有任何辦法知道那是規則而不是故障。規則必須在設計者正看著的地方、在
他伸手拿那個元件的那一刻說出來。

**它放在哪裡。** **Work** 與 **Build Firmware** 之間——挨著 Work，因為兩者都是關於整個
專案，而不是某一次操作的逐字稿。每個分頁都有，和 Work 一樣。

**它會自己跳到前面——但絕不自己打開抽屜。** 選取一個影片會讓 Information 成為頁簽列最
前面的那一頁，所以下次打開抽屜時等在那裡的就是它；收著的抽屜維持收著。選取元件是設計者
最常做的動作，一個每點一下就打開的抽屜，等於為了一段他隨時可以低頭看的說明把畫布從他
手上拿走。只在*新的*選取時：設計者在同一個影片還選著的時候切去 Work，那是他做的選擇，這個 pane
不可以把它撤銷。選別的東西時抽屜留在原地——對其他每一種元件，這個 pane 只會說
*nothing to add*，而一個沒話說的 pane 沒有跳到前面的道理。

**兩種句子。** *規則*是固定的——影片遵守的那八條，每次都一樣。*警告*是從畫面推導出來的：
`Button_2 overlaps this video`、`The video runs off the screen`、`No file named yet`。
重疊檢查把畫面的樹攤平成絕對座標的方框再求交集，略過影片自己的祖先（裝著影片的容器不算
「壓在」它上面）和隱藏的元件。工具列上一句結論——*Ready to play* 或 *2 reminders*——是
一眼就能看到的答案。是提醒，不是故障：這裡沒有任何一項會擋下建置，用字必須說清楚這一點。

**用設計者的話，而且只用設計者的話。** 這個 pane 裡沒有任何地方提到 LVGL、圖層、編解碼器
或緩衝區；有一個測試確保這件事。顯示控制器的第二層是規則存在的原因，而規則——*影片需要
獨佔自己的空間*——才是設計者要照著做的事。原因見 [video-playback.md](./video-playback.md)，
措辭見 [`componentNotes.ts`](../../src/components/DockPanel/componentNotes.ts)。

**接下來加什麼。** 任何一種在面板上的行為從畫布讀不出來的元件都屬於這裡：
`componentNotes.ts` 是放它的規則與檢查的唯一位置，pane 依類型自動撿起來。
