...
Preview 分頁為什麼有三個子分頁、它們的差別在哪、**Build & Run** 跟 **Deploy** 是什麼
...
| **要什麼工具鏈** | 不用 | 不用——`.wasm` 是簽進 repo 的 | dev server 上要有 `emcc` | CMake + Ninja + arm-none-eabi + STM32_Programmer_CLI |
| **延遲** | 即時、連動 | 連動，300 ms debounce | 數秒到數十秒 | 數分鐘 |
| **互動** | 模擬點擊、畫面切換、動畫 | 沒有 | 滑鼠與鍵盤轉進 LVGL | 面板自己的觸控 |
| **實作在** | `Preview/PreviewPanel.tsx`，2038 行 | `WasmPreview/WasmPreview.tsx`，114 行 | `CompilePreview/`，447 行＋`compilerService.ts` | `DeployPanel.tsx` ＋ `server/hmi/` |
...
圖片 C 陣列與 `lv_font_conv` 轉出來的字型，一起丟給 `POST /api/compile`，
而 `vite-plugin-compile.ts` 會叫出 **`emcc`**，把這些東西跟從原始碼建起來的 LVGL 一起
...
**它需要：** 跑 dev server 的那台機器上要有 `emcc`。`vite-plugin-compile.ts` 開頭那些
工具鏈路徑是絕對路徑，所以工具鏈不在的地方，這一階會回報編譯錯誤，而不是降級運作
（[language-switching.md](./language-switching.md) §4.1）。

## 5. 🚀 Deploy——而且不，它並沒有讓 Build & Run 變多餘

會這樣問很合理，因為兩者確實共用了一個真實的東西：**同一個 `generateCode()`**。
`server/hmi/projectSource.ts` 是從 `src/codegen/generator` import 它的，跟
`CompilePreview` 在瀏覽器裡做的一模一樣。這個專案只有一個程式碼產生器，被兩個地方
...
| LVGL 設定 | `wasm/lv_conf.h`（＋`generateCustomLvConf()`） | `firmware/<board>/include/lv_conf.h` |
...
所以兩者回答的是不同的問題。Build & Run 回答的是*「我產生出來的東西會動嗎？」*——
...
- codegen **不會**被 Code 分頁擋住。在工廠模式下把那個分頁藏起來，`generateCode()`
  仍然為 Build & Run 與這一階在跑（[factory-dev-mode.md](./factory-dev-mode.md)）
  ——會呼叫它的就是這兩階。
- `Build & Run` 可以在建置時整個關掉——`VITE_ENABLE_COMPILE_PREVIEW=false`
...
呼叫者是 `CodePanel`、`CodePreview`、`CompilePreview` 與 `server/hmi/projectSource.ts`
——而其中前兩個正是那個旗標會藏起來的 Code 分頁。撐過那個旗標的是 Build & Run 與
Deploy 建置，而它現在就是這樣寫的。

**[language-switching.md](./language-switching.md) §4.3 的涵蓋度表只有三欄，但階梯有
四階。** 它比較的是「Canvas 🌐」、Build & Run 與硬體，漏掉（或早於）LVGL Preview。
...
*LVGL Preview 被嚴格支配。* 它的獨門主張是「不用工具鏈的真 LVGL」——但 Build & Run
...
**這個答案附帶的一張帳單。** 價值最高的那一階，同時也是最可能一拿到手就是壞的那一階。
`vite-plugin-compile.ts:44` 把工具鏈預設成了別人的 Linux 工作區：
...
兩者都可以用環境變數覆寫，所以這是**設定問題，不是架構問題**——但這也表示「留
Build & Run」跟「把 `emcc` 這條路變可靠」是**同一個決定**，不是兩個。只選前者、不付
後者的帳，等於讓產品唯一真正的驗證階一直在回報編譯錯誤。

**唯一會翻轉這個答案的條件。** 如果那張帳單真的付不起——例如要出貨給永遠不會有工具鏈
的終端使用者——那就改留 **LVGL Preview**，因為它是唯一「真 LVGL＋簽進 repo 的二進位檔
＋零設定」的一階。那是次佳解，不是首選。

**Deploy 不在候選名單裡。** 它不是預覽，它是產品的輸出路徑。順帶一提，留著 Build & Run
也會讓 Deploy 保持誠實：兩者共用 `generateCode()`（§5），所以 **Build & Run 是唯一每天
在操 deploy 那條路徑的東西。**
