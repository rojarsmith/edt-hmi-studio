...
4. 成功後輸入欄關閉，**Factory Dev Mode** 標記會同時出現在 About 畫面與選單列
...
也可以隨時手動離開：點選單列的 **Factory Dev Mode** 標記並確認，編輯器會立即回到
一般狀態，要再進入需重新輸入密碼。
...
隱藏分頁並不會停用程式碼生成本身：`generateCode()` 在 WASM 預覽、Build & Run 流程與
專案匯出時照常執行。被管制的只是顯示產出原始碼的那個分頁。
