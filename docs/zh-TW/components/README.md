...
「本身就是一張圖」的元件，由內容計算而來——相對於 Basic 那個顯示專案匯入圖片的
Image。

| 元件 | 類型 | 圖示 | 預設尺寸 | 容器 | 說明 | 文件 |
|------|------|------|----------|------|------|------|
| QrCode | `qrcode` | 🔳 | 120×120 | ❌ | 由 Texts 素材庫資源（讀英語欄位）或字串產生的 QR 碼，執行期可由通訊傳入的字串取代。版本 1–40 或自動、縮放、四個錯誤更正等級 | [qrcode.md](qrcode.md) |
