...
Video 不是容器元件（`isContainer = false`），不能包含子元件——執行期只會把自己的訊息
標籤放進去，畫面則顯示在它之上的硬體圖層裡。
...
>
> **請把元件設成影片自己的解析度。** 畫面以自己的尺寸顯示在一個無法縮放的硬體圖層上：
> 比它大的元件四周是黑的，比它小的元件顯示中間。見[§12](#12-設計說明)。
...
什麼都放不進去。執行期只會自己建立一個子物件——一個 `lv_label` 承載面板要說的話——
而畫面顯示在 LVGL 畫的一切之上的硬體圖層裡，所以編輯器放進去的子元件會在影片底下，
而不是上面。
...
| API | 說明 |
|---|---|
| `board_display_overlay_show` / `_hide` | 解出來的影格，放在 LTDC 的第二個硬體圖層上——從來不是 LVGL 的圖片。見 [docs/zh-TW/video-playback.md §4](../video-playback.md#4-一格畫面怎麼變成像素) |
| `lv_obj_get_coords` | 元件的框在哪裡，好把覆蓋層對準它 |
...
2. **把元件設成影片的大小。** 畫面以自己的尺寸顯示在一個 LTDC 無法縮放的硬體圖層上：
   比它大的元件四周是黑的，比它小的元件顯示中間。而因為那一層在 LVGL 畫的一切之上，
   **播放中的影片上面畫不了任何東西**——說明文字放在旁邊。
