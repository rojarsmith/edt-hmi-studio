...
const heroBuf = fs.readFileSync(path.join(__dirname, "hero-vivid.png"));
const shapesBuf = fs.readFileSync(path.join(__dirname, "shapes-vivid.png"));
const iconBuf = (n) => fs.readFileSync(path.join(__dirname, `icon-vivid-${n}.png`));

// vivid palette
const INK = "1F2937";       // deep slate ink
const SLATE = "64748B";     // secondary text
const CORAL = "F4633A";     // headline + chip
const per = {
  design:  { title: "1E40AF", bg: "EAF1FE" },  // blue
  logic:   { title: "5B21B6", bg: "F1EBFC" },  // violet
  protocol:{ title: "0F766E", bg: "E4F4F1" },  // teal
  preview: { title: "B45309", bg: "FBF1DE" },  // amber
  deploy:  { title: "1F4A2C", bg: "E9F2EA" },  // brand green
};
...
function card(key, title, desc) {
  return new TableCell({
    ...
    shading: { type: ShadingType.CLEAR, fill: per[key].bg },
    borders: noBorders,
    children: [
      new Paragraph({
        spacing: { after: 120 },
        children: [new ImageRun({ type: "png", data: iconBuf(key), transformation: { width: 30, height: 30 } })],
      }),
      new Paragraph({
        spacing: { after: 90 },
        children: [new TextRun({ text: title, font: "Segoe UI Semibold", size: 26, bold: true, color: per[key].title })],
      }),
      new Paragraph({
  ...
  sections: [
    {
      ...
      children: [
                  ...
                  children: [
                    new Paragraph({
                      ...
                      children: [
                        ...
                        new TextRun({ text: "   EDT HMI STUDIO", font: "Segoe UI Semibold", size: 21, bold: true, color: SLATE, characterSpacing: 50 }),
                      ],
                    }),
                    new Paragraph({
                      spacing: { after: 40 },
                    ...
                    new Paragraph({
                      spacing: { after: 60 },
                      children: [new TextRun({ text: "No code.", font: "Segoe UI", size: 108, bold: true, color: CORAL })],
                    }),
                    new Paragraph({
                      spacing: { after: 180 },
                      children: [new TextRun({ text: "Not one line.", font: "Segoe UI Light", size: 108, color: "94A3B8" })],
                    }),
                    new Paragraph({
                      spacing: { after: 220 },
                      children: [
                        new TextRun({
                          ...
                          font: "Segoe UI", size: 26, color: SLATE,
                        }),
                      ],
                    }),
                    new Table({
                      ...
                      rows: [
                        new TableRow({
                          children: [
                            new TableCell({
                              width: { size: 4700, type: WidthType.DXA },
                              shading: { type: ShadingType.CLEAR, fill: CORAL },
        ...
        new Paragraph({
          ...
          children: [
            new TextRun({ text: "FIVE STEPS FROM IDEA TO MACHINE  \u00B7  ZERO CODE", font: "Segoe UI Semibold", size: 20, bold: true, color: SLATE, characterSpacing: 60 }),
          ],
        }),
              ...
              children: [
                new TableCell({
                  ...
                  children: [
                    new Paragraph({
                      children: [
                        new TextRun({ text: "EDT HMI Studio \u00B7 v1.0  \u2014  from idea to a running touchscreen, without writing a single line of code.", font: "Segoe UI", size: 19, color: SLATE }),
                      ],
                    }),
                  ],
                }),
...
Packer.toBuffer(doc).then((buf) => {
  const out = path.join(__dirname, "..", "edt-hmi-studio-teaser-a4-vivid.docx");
