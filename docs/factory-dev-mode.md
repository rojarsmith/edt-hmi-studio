...
| **Emulator wording** — the status line, the canvas placeholder, the first-run note, the not-set-up panel and the Build Output pane. In factory dev mode they name LVGL, Emscripten, emcc and the compiler's own transcript, because that is what is worth reading when working on the editor. Normally they say what happened in the product's words — *Preparing your panel*, *Running — click the screen to try it* — and the Build Output pane is filtered through the same allow-list the Work pane uses, so a compiler transcript never reaches someone who did not ask for one. The not-set-up panel changes audience too: the engineer gets the missing pieces by name and the command that installs them, the panel designer gets who to ask and what to do meanwhile | engineering wording | the product's words |
