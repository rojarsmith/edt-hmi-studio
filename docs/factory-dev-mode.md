...
| **Emulator wording** — the status line, the canvas placeholder, the first-run note and the not-set-up panel. The status line beside the buttons is factory-only except for a failure: normally the Start button, the "Working…" label and the sentence over the canvas already say what state it is in, and repeating that beside them is noise — a failure is the one state nothing else reports. In factory dev mode they name LVGL, Emscripten and emcc, because that is what is worth reading when working on the editor. Normally they say what happened in the product's words — *Preparing your panel*, *Running — click the screen to try it* — and the not-set-up panel changes audience with them: the engineer gets the missing pieces by name and the command that installs them, the panel designer gets who to ask and what to do meanwhile. The **Build Output** pane is deliberately not on this list; it keeps its detail in both modes and is expected to move behind this flag whole ([bottom-dock-panel.md](bottom-dock-panel.md) §11) | engineering wording | the product's words |
| **Emulator footer** — the strip under the Emulator's canvas naming the LVGL version and the compiler that built it. It answers "why does this differ from the board", which is a question only someone working on the editor asks; the panel above it says everything a user needs, including what is missing and how to install it | shown | hidden |
