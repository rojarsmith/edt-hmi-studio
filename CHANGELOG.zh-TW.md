...
### 變更
- **Logic 圖有了自己的管理面板，位於節點清單下方** —— 原本的圖選擇器是浮在畫布上的下拉選單加一顆垃圾桶按鈕：既擋畫面，又稱不上管理。現在 Nodes 面板下方有一個可收合的 Logic Graphs 面板 —— 搜尋、新增、刪除、點選開啟，開啟中的圖有高亮標示，收合開關領在標題列最前面。刪除會以名字確認。新增時拒絕與既有圖重複的名字，並帶著被拒的名字重開命名框 —— 清單裡兩個同名的圖是分不出來的；預設建議名稱照舊自動跳過已被占用的
