...
### 新增
- **Video 現在播的是播放清單，檔案可以在卡片上的任何位置** —— 原本的單一檔名長成了三件事。每一項前面可以帶**資料夾**，正反斜線都收，因為 Windows 使用者照自己螢幕上看到的打。元件收一份**檔案清單**，一行一個、照播放順序——或者掃描一個**資料夾**（預設卡片最上層），把裡面每一個 .avi 照名稱排序播出來，最多 64 個，掃描發生在開始播放時，所以重插的卡片會被撿到。多檔清單預設**照順序**播；**Random order** 用軟體亂數序列挑下一個，每次開機種子都不同，而且絕不會挑到剛播完的那一個——Loop 關閉時清單有幾個就播幾部然後停。打不開的檔案**跳過，不是致命錯誤**：播放清單是一組各自獨立的影片，只有每一個成員都失敗時元件才說明原因。Loop 現在指的是整份清單。播放清單出現之前存的專案帶著舊的單一 `fileName`，照舊播放——每一層都經過同一個正規化器讀取。產生的程式碼交給韌體的是一張常數播放清單表而不是一個名字，編輯器的檢查也跟著長大：副檔名錯了會指名是哪一項、重複列出的檔案會被點出來，資料夾掃描則留給面板，因為只有它真的能看
