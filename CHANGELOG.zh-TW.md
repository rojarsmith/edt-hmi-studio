...
### 新增
- **QrCode 可以用一個不是它內容的字串來規劃**——由通訊餵內容的碼在設計期沒有內容，所以沒有東西告訴設計者最長的工單網址需要哪個版本、在這個 scale 下裝不裝得進框、綁定要讀幾個 register。**Plan for a string** 就是為此而設的欄位：打進伺服器將來會送的最長字串——Unicode 歡迎，以 UTF-8 位元組計算，和碼與 register 的算法一致——編輯器就回答字元數與位元組數（漢字讓兩者不同時會提醒）、元件目前等級下的最小版本與其模組數和像素、*四個*等級各自的最小版本（多一級錯誤更正要付多少代價一眼就看到，不用試四次）、字串綁定需要的 register 數，以及——只在有問題時——該改什麼、改成多少：*版本設成 3 或更高*、*scale 降到 3 或元件放大到 259×259*、*綁定的 Length 提高到 16*、*超過通訊能承載的長度*。只用於規劃：絕不編碼（打字時畫布保持空白）、絕不進產生的程式碼、絕不送到 Simulator——各有一個測試把門守住——但會隨專案儲存在元件的 `sampleText` 裡，下一個人就看得到這個碼是按什麼尺寸規劃的。見 [docs/zh-TW/components/qrcode.md](docs/zh-TW/components/qrcode.md)
- **QrCode 元件，放在 Miscellaneous 之上新的 Image 類別裡** —— 由內容計算出來的 QR 碼，用 QR 標準自己的名字暴露標準自己的旋鈕：版本 1–40 或 Auto（裝得下的最小版本）、每模組像素數的縮放、以及全部四個錯誤更正等級。內容可以來自 **Texts 素材庫的資源，一律讀英語欄位**——不管面板顯示哪種語言，因為 QR 碼是給對著它的手機看的，不是給操作員讀的，碼背後的網址不會翻譯——或是直接打在元件裡的**字串**（預設 `https://bitdove.net`）；兩者之上，**通訊傳入的字串會在執行期取代內容**：元件綁定一塊 1–64 個連續 registers，每個載兩個 ASCII 字元、高位元組在前、以零結尾——這是一個從頭走到底的新 `string` 資料型別：綁定編輯器、綁定產生器、以及每塊板子執行期裡的一條文字路徑，而且**只在字串真的變了才重新編碼**。每一層畫的都是同一個碼：設計畫布把真實內容編成 SVG、Prototype 用 Canvas 2D 矩形、Simulator 與產生的韌體跑的就是 LVGL 內附的那個 `qrcodegen` 編碼器——直接呼叫，因為 LVGL 自己的 `lv_qrcode` 包裝把更正等級釘死在 MEDIUM、版本也自己挑，而這兩者正是這個元件交給使用者的設定。屬性面板在底部那行替你算——內容需要哪個版本、那是幾像素、放不放得進元件的框——並多了一個 **quiet zone 開關**（標準的 4 模組淨空，預設開啟；關掉時會提醒掃描器需要淨空）和一鍵 **Shrink the widget to the code** 按鈕，把框縮到碼的精確像素——因為碼四周的白色其實是兩層：安靜區，加上元件自己的背景在填滿剩下的框——釘死的版本裝不下內容時是一則帶著解法的錯誤，面板則保留上一張圖，而不是畫半個碼。純軟體，所以和 Video 不同，每塊板子都能建置；四份 LVGL 設定的 `LV_USE_QRCODE` 都已開啟。見 [docs/zh-TW/components/qrcode.md](docs/zh-TW/components/qrcode.md)
- **Video 現在播的是播放清單，檔案可以在卡片上的任何位置** —— 原本的單一檔名長成了三件事。每一項前面可以帶**資料夾**，正反斜線都收，因為 Windows 使用者照自己螢幕上看到的打。元件收一份**檔案清單**，一行一個、照播放順序——或者掃描一個**資料夾**（預設卡片最上層），把裡面每一個 .avi 照名稱排序播出來，最多 64 個，掃描發生在開始播放時，所以重插的卡片會被撿到。多檔清單預設**照順序**播；**Random order** 用軟體亂數序列挑下一個，每次開機種子都不同，而且絕不會挑到剛播完的那一個——Loop 關閉時清單有幾個就播幾部然後停；只指名一個檔案時這個開關是灰的，因為一個檔案沒有順序可以打亂。打不開的檔案**跳過，不是致命錯誤**：播放清單是一組各自獨立的影片，只有每一個成員都失敗時元件才說明原因。Loop 現在指的是整份清單。播放清單出現之前存的專案帶著舊的單一 `fileName`，照舊播放——每一層都經過同一個正規化器讀取。產生的程式碼交給韌體的是一張常數播放清單表而不是一個名字，編輯器的檢查也跟著長大：副檔名錯了會指名是哪一項、重複列出的檔案會被點出來，資料夾掃描則留給面板，因為只有它真的能看
- **抽屜裡多了一個 Information pane，而且選到影片時它會自己出聲** —— **Work** 與 **Build Firmware** 之間的第五個 pane，也是第一個不是 log 的：它用放元件的人的話，說被選取的元件需要面板提供什麼。影片在面板上會遮住所有跟它重疊的東西、以錄製時的尺寸而不是元件的尺寸顯示、播的是編輯器從來沒看過的卡片上的檔案——這些畫布上統統看不到，畫布上的影片只是一塊寫著檔名的黑框。第一個放了影片的專案把一顆按鈕疊在上面，面板顯示了影片卻沒有按鈕。所以現在選取影片會讓 Information 成為頁簽列最前面的那一頁——但不會把收著的抽屜打開，因為選取元件是設計者最常做的動作，一個每點一下就開的抽屜等於為了一段說明把畫布拿走——列出影片遵守的八條規則，而規則上面是*這一個*影片在*這一個*畫面上違反了什麼：`Button_2 overlaps this video`、`The video runs off the screen`、`No file named yet`，工具列上附一句 *Ready to play* / *2 reminders* 的結論——是提醒不是故障，這裡沒有任何一項會擋下建置。重疊檢查把畫面攤平成絕對座標的方框，略過影片自己的容器和隱藏的元件。只在新的選取時才跳出來，所以同一個影片還選著時切去 Work 不會被撤銷；選其他任何元件都不動抽屜，因為對它們這個 pane 沒話可說。裡面沒有任何地方提到 LVGL、圖層、編解碼器或緩衝區，而且有測試守著。見 [docs/zh-TW/bottom-dock-panel.md](docs/zh-TW/bottom-dock-panel.md) §12
- **從 SD 卡播影片的 Video 元件** —— 元件面板多了一個 **Miscellaneous（雜項）** 分類，裡面放著唯一一個內容不屬於專案的元件。其他每一個元件都會被編進韌體映像檔；而一分鐘的 800x480 影像大約 200 MB，是那塊要播它的板子內部 Flash 的兩百倍。所以 Video **指名**它的檔案而不是帶著它：你打上 `intro.avi`，面板在執行期自己到 SD 卡根目錄把那個檔案找出來，而關於它的任何東西都不會進入建置。一段兩小時的片子花掉的韌體空間，和兩秒鐘的一模一樣——零——而要換片就換卡，不必重新建置。它只有三個屬性，沒有更多：檔名、**Auto Play** 與 **Loop**。格式是 **AVI 容器裡的 Motion JPEG**，而這不是偏好：它是唯一一種每格工作量剛好就是 STM32H747 的 JPEG 硬體在做的事的常見格式，而 DMA2D 直接讀那顆硬體輸出的 MCU 區塊、寫出掃描線順序的 ARGB8888，所以像素路徑上完全沒有 CPU。24 fps 下一格是 41 ms，而解一格只花幾毫秒，主迴圈在兩格之間照樣跑 Modbus 與觸控。也因為播影片需要那顆硬體與 SD 介面，它是第一個「能不能用」取決於**矽晶片**而不是取決於「驅動還沒寫」的元件——一顆同時還要畫 UI 的 Cortex-M7 不可能用軟體一秒解 24 張 JPEG，根本沒有一條比較慢的路可以退。所以它的把關方式和「尚未實作的通訊協定」完全一樣：元件在任何板子上都能放、能設定、能預覽、能存檔，而用到它的專案在 F746G-DISCO 與 EVK043027B 上**不能建置**，Video 區塊與 Deploy 分頁都會說明原因。沒有影片的專案不用為此付一毛錢——`--gc-sections` 會把整份執行期與它那 3.1 MB 的 SDRAM 緩衝區一起丟掉；實測結果是有影片的專案**多 20 KB Flash**，沒有影片的專案逐位元組不變。面板播不動時說的話，也保持在「使用者能據此行動」的精確度上：卡片上沒有那個名字時是 **Video not found**，但插槽是空的時候是 **No SD card**、卡在裡面而檔案系統不在時是 **SD card unreadable**——因為在人家只是忘了把卡推進去的時候告訴他影片不見了，會把他送去錯的地方找。設計畫布、Prototype 與 Emulator 都不播任何東西——它們都沒有面板的讀卡機，也從來沒看過那個檔案——所以三者都畫出畫面將要佔住的那個框、把檔名寫在上面，而不是憑空捏造一張劇照。韌體那一側在 H747I 上新增了四個模組：一個串流走 `movi`、完全不讀索引的 AVI 解多工器，所以記憶體不隨片長增長；JPEG 與 DMA2D 的解碼；一份跑在板子 microSD 插槽上的唯讀 FatFs，`disk_*` 直接對著 BSP 實作、`FF_FS_READONLY` 打開，所以這份韌體做的任何事都改不了一張卡片；以及元件執行期，它在元件真的出現在畫面上之前什麼都不開。音訊會被跳過、不播——這件事以及另外四件刻意不做的事，見 [docs/zh-TW/video-playback.md](docs/zh-TW/video-playback.md) §7
...
### 修正
- **沒有內容可編碼的 QR 碼是空白，不是「什麼都沒有」的碼**——產生的繪製器把 context 裡的東西原樣交給 `qrcodegen`，而空字串編起來毫無問題：一個合法但無意義的 version 1 碼，開機就出現在面板上，手機掃了什麼也做不了。Emulator 在這種情況本來就留白；現在韌體與設計畫布也一樣——只剩元件自己的背景色，一個素色方塊，直到設了內容或通訊送來字串。範例網址 `https://bitdove.net` 也基於同樣理由從元件預設值拿掉：新的 QrCode 一開始是空的，屬性面板的 *Nothing to encode yet* 是提示而非警告，因為開機空白正是由通訊餵內容的碼該有的起始狀態
- **面板現在真的會去讀 QR 碼的字串了——Modbus client 以前一次只能帶兩個 register** ——每塊板子的 `modbus_rtu_async_client` 都還留著 `MODBUS_RTU_ASYNC_MAX_REGISTERS 2`，那是元件最寬只讀 32 位元數字時代的上限。字串綁定一次要讀最多 64 個，client 在**任何位元組離開 UART 之前**就拒絕了這筆讀取，每次輪詢都這樣，而且不吭聲：編輯器看得到綁定、產生的描述子正確、執行期的文字路徑正確，測試伺服器的儀表板卻只看到面板在讀 Label 的單一 register，從來沒讀過字串區塊——就是這樣找到的。讀取上限現在是 64 個 register（128 位元組 UTF-8，就是編輯器提供的範圍；遠低於協定的 125），回應緩衝區由這個數字算出而非手打，寫入維持自己的兩個 register 上限，請求緩衝區不跟著變大。64 個 register 的回應在 9600 baud 要 170 ms 線上時間——超過預設 250 ms 回應逾時的一半——所以接收期限現在是逾時加上回應本身的線上時間，而不是和它賽跑。一個主機端測試讀三份韌體，把 client 的上限、執行期的上限和編輯器的上限綁在一起
- **Work 頁簽的紅燈現在只要有未完成的項目就會亮，不再只看韌體** —— [docs/zh-TW/work-progress.md](docs/zh-TW/work-progress.md) §6 一直寫著「只要有任何東西在跑，Work 頁簽就有一顆紅燈」，但它實際接的是 deploy store，而那只知道韌體建置與燒錄兩件事。於是第一個從別處加入 Work 的操作——Emulator 的建置——跑起來的時候燈是暗的。它現在直接數 Work 裡還在跑的項目，tooltip 會講出正在跑的是誰、或是有幾個
