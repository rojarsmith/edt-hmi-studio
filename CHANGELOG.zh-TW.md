...
### 新增
- **Logic 圖可以指定它在哪些 Screen 作用** —— 右側 Variables 上方多了一個 Properties 面板，與左下的圖清單同款的可收合形狀，顯示目前開啟的圖的設定。Active on Screens 預設全部；取消「全部」會從「每個 Screen 都還勾著」的明細清單開始，排除一個畫面只要一下，而不是整份重建；一個都不勾時會明講這張圖不會在任何畫面跑。設定存在圖本體上，隨存檔與匯出保留。產生的程式碼還沒有依它篩選 —— 設定是先立的契約，篩選是接下來的事
...
### 修正
- **Event Trigger 的圖在硬體上從來不會執行 —— 對話框從不問是哪個元件觸發** —— 整條鏈本來就存在而且有測試（資料模型帶著目標元件欄位、codegen 會產生 `lv_obj_add_event_cb`、每塊板的韌體樣板都在正確時機呼叫 `ui_logic_init()`），但 Edit Node 對話框只給 Event Type，於是註冊那一行永遠不會產生，每一張事件圖都是死碼。對話框現在有了兄弟節點一直都有的那個 Target Component 選擇器，還沒選時會明講後果。節點本體也說得出是誰：選定元件的名字顯示在事件那行上方，而目標元件事後被刪除的會讀作 `Target: (missing)`，不讓 codegen 無聲地退回名稱推導。**Event Object** 輸出在同一次變更移入原廠人員研發模式：產生的程式碼仍會丟棄事件，一般模式下這個埠只承諾裝置給不了的東西 —— 已經接了線的埠在兩種模式都顯示，連線永遠不會被藏斷。調查過程與兩個決定都記錄在 [docs/logic-event-trigger.md](docs/zh-TW/logic-event-trigger.md)
