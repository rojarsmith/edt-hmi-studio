...
function project(actionType: 'playAnimation' | 'stopAnimation', animationId: string, animOverrides = {}) {
  const { component: target, animations } = animatedComponent(
    'label',
    { id: 'target', name: 'Title' },
    { id: 'anim-1', name: 'Pulse_1', property: 'opa', ...animOverrides },
  );
  ...
  return {
    screens: [createScreen({ name: 'main', components: [target, button] })],
    animations,
  };
}

const eventsFor = (p: ReturnType<typeof project>, options: ReturnType<typeof defaultOptions>) =>
  generateEventsSource(p.screens, options, [], [], p.animations);

describe('animation actions', () => {
  it('plays the named animation by calling its start function', () => {
    const result = eventsFor(project('playAnimation', 'anim-1'), defaultOptions());
  ...
  it('stops it by calling its stop function', () => {
    const result = eventsFor(project('stopAnimation', 'anim-1'), defaultOptions());
  ...
  it('names the same symbol ui.c defined, so the call links', async () => {
    ...
    const p = project('playAnimation', 'anim-1');
    const source = generateUiSource(
      p.screens, defaultOptions(), undefined, [], undefined, undefined, [],
      undefined, undefined, undefined, [], [], p.animations,
    );

    expect(source).toContain('void ui_anim_pulse_1_start(void) {');
    expect(eventsFor(p, defaultOptions())).toContain('ui_anim_pulse_1_start();');
  });
  ...
  it('comments out a binding whose animation is gone', () => {
    ...
    const result = eventsFor(project('playAnimation', 'deleted-id'), defaultOptions());
  ...
  it('comments out a binding whose property cannot be animated', () => {
    const result = eventsFor(project('playAnimation', 'anim-1', { property: 'bg_color' }), defaultOptions());
  ...
  it('survives a binding that names no animation at all', () => {
    const result = eventsFor(project('playAnimation', ''), defaultOptions());
