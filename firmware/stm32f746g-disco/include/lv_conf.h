 ...
 *
 * So the heap moved out of internal RAM, which never had that much, into the
 * .sdram section of the linker script - 7 MB of external SDRAM above the
 * frame buffers. It is CPU-only memory, no DMA reads it, so the D-cache
 * needs no maintenance for it; the FMC bandwidth it shares with the LTDC is
 * the price of a rotation being possible at all.
 */
