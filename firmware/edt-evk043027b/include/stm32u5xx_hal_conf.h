...
/*
 * Taken from the EVK043027B vendor package and trimmed to the modules this
 * firmware actually links: ADC, CORDIC, CRC, DMA2D, FDCAN, GPU2D, PCD and SD
 * are switched off here because nothing in the HMI runtime drives them. Turn
 * one back on and add its .c to CMakeLists.txt together — enabling a module
 * only pulls in its header, so a missing source shows up as a link error at the
 * far end of a long build.
 *
 * HSE_VALUE below is the board's 25 MHz oscillator and is what every clock in
 * board.c is derived from; it is not a placeholder.
 */
